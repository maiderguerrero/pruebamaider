...
(function (blink) {
	...
	var pruebamaiderStyle = function () {
			blink.theme.styles.basic.apply(this, arguments);
		},
		page = blink.currentPage;

	pruebamaiderStyle.prototype = {
		bodyClassName: 'content_type_clase_pruebamaider',
		ckEditorStyles: {
			name: 'pruebamaider',
			styles: [
		...
		formatCarouselindicators: function () {
			...
			$navbarBottom
				.attr('class', 'pruebamaider-navbar')
		...
		animateNavbarOnScroll: function () {
			...
			var $navbar = $('.pruebamaider-navbar');
			var lastScrollTop = 0;
        ...
        changeHighBar: function () {
            if($('.pruebamaider-navbar').length>0 && $('.navbar').length>0){
                blink.theme.setTopByHeight('.navbar', '.pruebamaider-navbar');
            }
        }
	};
